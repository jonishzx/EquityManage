...
<span class="radiobuttonlist">
    <% if (!enabled && value != null)
       {
           var it = rst.Find(x => value.ToString() == x.Code);
           if(it!=null) %>
        <%= rst.Find(x => value.ToString() == x.Code).Name %>
        <input id="<%= id %>" style="display: none;"  name="<%= id %>" type="radio" checked="checked" value="<%= value %>" />
    <% }
       else
       {
           var i = 0;
           ...
           {
               i++;
    %><input id="<%= id + "_" + i %>" <%= !enabled ? "disabled" : "" %> name="<%= id %>" style="width:auto !important;" <%= ViewData.Eval("Attr") %> value='<%= m.Code %>'  <%= (value != null && value.ToString() == m.Code) ? "checked=\"checked\"" : "" %> type=radio /><label for="<%= id + "_" + i %>"  style="width:auto !important;"><%= m.Name %></label>       
    <% }
       }
   } %>
</span>