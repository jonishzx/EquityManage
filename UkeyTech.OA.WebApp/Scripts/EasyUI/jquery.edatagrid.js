 ...
 * 
 */
function destoryrow(dg, row, opts) {
    var index = dg.datagrid('getRowIndex', row);
    dg.datagrid('cancelEdit', index);
    if (row.isNewRecord) //未保存的记录
    {
        dg.datagrid('cancelEdit', index);
...
(function ($) {
    function buildGrid(target) {
        ...
        $(target).datagrid($.extend({}, opts, {
            ...
            onAfterEdit: function (index, row) {
                ...
                opts.onAfterSave.call(target, index, row);
        ...
        if (opts.autosave) {
            $(target).datagrid("getPanel").mouseleave(function () {
                setTimeout(function () {

                    $(target).edatagrid("saveRow");

                }, opts.autosaveTimeout);
            });
        }
    ...
    $.fn.edatagrid.methods = {
        ...
        saveRow: function (jq) {
            return jq.each(function () {
                var dg = $(this);
                var edg = $.data(this, 'edatagrid');
                if(edg){
                    var opts = edg.options;
                    if (opts.onBeforeSave.call(this, opts.editIndex) == false) {
                        setTimeout(function () {
                            dg.datagrid('selectRow', opts.editIndex);
                        }, 0);
        ...
        destroyRow: function (jq) {
            ...
            return jq.each(function () {
                ...
                if (!rows && opts.editIndex == undefined) {
                    $.messager.show({
                        title: opts.destroyMsg.norecord.title,
                        msg: opts.destroyMsg.norecord.msg
                    });
